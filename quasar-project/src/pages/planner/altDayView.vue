...
methods:{
    ...
    onLoadDefault(){
      ...
      if (this.daSchedule.getAllEvts().length > 0){
        let e = this.daSchedule.unscheduledDefaults()
        console.log('onLoadDefault::unscheduledDefaults', e?.length) //e
    ...
    oneToManyConflict(conflicts,override = null,from=''){
      ...
      let defaultOpts = [{ label: 'Pick one event', value: 'opt3' }]
        ...
        const forceAdd = (evt,timey) => { //should pass in all conflicts...todo**
          ...
          let o = this.daSchedule.recurChangeTime(aSched.id,evt,timey,true) //from != 'onDrop'
        ...
        const forceAdd = (conflictEvts) => {
          ...
          conflictEvts.forEach((obj) => {
            ...
            if (currScheduled.id != obj.id){
              ...
              let euhOverlaps = this.daSchedule.recurChangeTime(currScheduled.id,obj,timey, true) //from != 'onDrop'
    ...
    onPickEvent(addE,skipAsk,useBalance) { 
      ...
      let isClose = this.daSchedule.tooClose(this.targetDrop.timestamp, addE.duration) 
    ...
    onAddHocEvent(goalTitle, parentGoal, own, interval,useBalance){
      ...
      if (subID) {
        ...
        let res = this.daSchedule.onAdHocEvent(subID,timeStart,useBalance)
